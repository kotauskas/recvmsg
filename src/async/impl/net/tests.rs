...
async fn udp(v6: bool) {
    ...
    let (mut s1, mut s2) = try_join!(
        UdpSocket::bind((addr, 0)), UdpSocket::bind((addr, 0)),
    ).expect("bind failed");
    ...
    try_join!(
        s1.connect((addr, p2)), s2.connect((addr, p1)),
...
the memory allocator decides to be smarter than usual and give us a huge buffer on the first try";
    ...
    let (ssz1, ssz2) = try_join!(s1.send(msg.as_bytes()), s2.send(msg.as_bytes())).expect("send failed");
    ...
    let (rslt1, rslt2) = try_join!(s1.recv_msg(&mut buf1), s2.recv_msg(&mut buf2)).expect("receive failed");
