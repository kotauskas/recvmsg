...
pub trait TruncatingRecvMsgExt: TruncatingRecvMsg {
    ...
    fn recv_trunc<'io, 'buf, 'slice>(
        &'io mut self,
        peek: bool,
        buf: &'buf mut MsgBuf<'slice>,
    ) -> RecvTrunc<'io, 'buf, 'slice, Self>
    where
        Self: Unpin,
    {
...
pub trait TruncatingRecvMsgWithFullSizeExt: TruncatingRecvMsgWithFullSize {
    ...
    fn recv_trunc_with_full_size<'io, 'buf, 'slice>(
        &'io mut self,
        peek: bool,
        buf: &'buf mut MsgBuf<'slice>,
    ) -> RecvTruncWithFullSize<'io, 'buf, 'slice, Self>
    where
        Self: Unpin,
    {
